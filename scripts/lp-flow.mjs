...
const PLUGIN_VERSION = await fs.readFile(path.join(PLUGIN_ROOT, '.codex-plugin', 'plugin.json'), 'utf8')
  ...
  .catch(() => '0.1.0');
const PLUGIN_ASSETS = path.join(PLUGIN_ROOT, 'assets');
...
function normalizePerformanceProfile(value, fallback = 'scientific') {
  const raw = normalizeString(value === undefined || value === null || value === '' ? fallback : String(value)).toLowerCase();
...
function buildMolstarViewerCommand(input) {
  const outDir = normalizeString(input.out_dir || input.outDir);
  const receptor = normalizeString(input.receptor);
  if (!outDir) throw new Error('out_dir is required');
  if (!receptor) throw new Error('receptor is required');
  const python = normalizeString(input.python) || 'python';
  const script = path.join(SERVER_DOCKING_SCRIPTS, 'make_molstar_viewer.py');
  const args = [script, '--out-dir', outDir, '--receptor', receptor];
  const references = input.reference_ligand || input.referenceLigand || input.reference_ligands || input.referenceLigands || [];
  for (const item of (Array.isArray(references) ? references : [references]).filter(Boolean)) {
    args.push('--reference-ligand', String(item));
  }
  const poses = input.pose || input.poses || [];
  for (const item of (Array.isArray(poses) ? poses : [poses]).filter(Boolean)) {
    args.push('--pose', String(item));
  }
  const topN = input.top_n || input.topN;
  if (topN) args.push('--top-n', String(topN));
  const caseName = normalizeString(input.case || input.caseName);
  if (caseName) args.push('--case', caseName);
  const title = normalizeString(input.title);
  if (title) args.push('--title', title);
  const assetDir = normalizeString(input.asset_dir || input.assetDir);
  if (assetDir) args.push('--asset-dir', assetDir);
  const activeSiteNote = normalizeString(input.active_site_note || input.activeSiteNote);
  if (activeSiteNote) args.push('--active-site-note', activeSiteNote);
  if ([input.debug_layout, input.debugLayout, input.show_left_panel, input.showLeftPanel].some(explicitTrue)) {
    args.push('--debug-layout');
  }
  ...
  return {
    ...
    note: 'This builds the command only. Run it from a shell with a real Python interpreter available.',
  };
}

function buildDockingStoryCommand(input) {
  const resultsDir = normalizeString(input.results_dir || input.resultsDir || input.run_dir || input.runDir);
  if (!resultsDir) throw new Error('results_dir is required');
  const node = normalizeString(input.node) || (typeof process !== 'undefined' && process.execPath ? process.execPath : 'node');
  const script = path.join(SERVER_DOCKING_SCRIPTS, 'make_docking_story.mjs');
  const args = [script, '--results-dir', resultsDir];
  const outDir = normalizeString(input.out_dir || input.outDir);
  if (outDir) args.push('--out-dir', outDir);
  const caseName = normalizeString(input.case || input.caseName);
  if (caseName) args.push('--case', caseName);
  const receptor = normalizeString(input.receptor);
  if (receptor) args.push('--receptor', receptor);
  const references = input.reference_ligand || input.referenceLigand || input.reference_ligands || input.referenceLigands || [];
  for (const item of (Array.isArray(references) ? references : [references]).filter(Boolean)) {
    args.push('--reference-ligand', String(item));
  }
  ...
  const topLigands = input.top_ligands || input.topLigands;
  if (topLigands) args.push('--top-ligands', String(topLigands));
  const posesPerLigand = input.poses_per_ligand || input.posesPerLigand || input.pose_limit || input.poseLimit;
  if (posesPerLigand) args.push('--poses-per-ligand', String(posesPerLigand));
  const center = normalizeString(input.center);
  ...
  return {
    ...
    argv,
    command_line: argv.map(shellQuote).join(' '),
    note: 'Builds a Mol View Stories source folder from docking results. The generated folder can be exported with upstream mvs CLI when available.',
  };
}

async function makeDockingStory(input) {
  const command = buildDockingStoryCommand(input);
  const result = await runCommand(command.argv, { timeout_ms: input.timeout_ms || input.timeoutMs || 120000 });
  return {
    ok: result.ok,
    executed: true,
    command,
...
function buildCellpackStoryCommand(input) {
  const model = normalizeString(input.input || input.model || input.bcif);
  if (!model) throw new Error('input is required');
  const node = normalizeString(input.node) || (typeof process !== 'undefined' && process.execPath ? process.execPath : 'node');
  const script = path.join(SERVER_DOCKING_SCRIPTS, 'make_cellpack_story.mjs');
  const args = [script, '--input', model];
  const mappings = [
    ['out_dir', 'outDir', '--out-dir'],
    ['output', 'output', '--output'],
    ['annotation_map', 'annotationMap', '--annotation-map'],
    ['title', 'title', '--title'],
    ['assembly_id', 'assemblyId', '--assembly-id'],
    ['cutaway', 'cutaway', '--cutaway'],
    ['clip_axis', 'clipAxis', '--clip-axis'],
    ['clip_offset', 'clipOffset', '--clip-offset'],
    ['max_group_scenes', 'maxGroupScenes', '--max-group-scenes'],
    ['performance_profile', 'performanceProfile', '--performance-profile'],
    ['performance_preset', 'performancePreset', '--performance-preset'],
    ['max_proxies', 'maxProxies', '--max-proxies'],
    ['proxy_radius_scale', 'proxyRadiusScale', '--proxy-radius-scale'],
    ['coordinate_unit', 'coordinateUnit', '--coordinate-unit'],
    ['guide_scale', 'guideScale', '--guide-scale'],
    ['builtin_validate', 'builtinValidate', '--builtin-validate'],
    ['force_cellpack', 'forceCellpack', '--force-cellpack'],
  ];
  for (const [snake, camel, flag] of mappings) {
    const value = input[snake] ?? input[camel];
    if (value !== undefined && value !== null && value !== '') args.push(flag, String(value));
  }
  const legacyValidate = input.validate;
  if (
    legacyValidate !== undefined
    && legacyValidate !== null
    && legacyValidate !== ''
    && input.builtin_validate === undefined
    && input.builtinValidate === undefined
  ) {
    args.push('--validate', String(legacyValidate));
  }
  const argv = [node, ...args];
  return {
    ok: true,
    ...
    argv,
    command_line: argv.map(shellQuote).join(' '),
    note: 'Runs conservative input classification before writing artifacts, then builds a portable metadata-driven CellPACK/Mesoscale-style MVSJ story. It does not reproduce full Mol* Mesoscale Explorer plugin state.',
  };
}

async function makeCellpackStory(input) {
  const command = buildCellpackStoryCommand(input);
  const result = await runCommand(command.argv, { timeout_ms: input.timeout_ms || input.timeoutMs || 180000 });
  return {
    ok: result.ok,
    executed: true,
    command,
...
function unsupportedCombination(message, details = {}) {
  return {
    ok: false,
    status: 'unsupported',
    message,
...
const PLANNED_OR_INTERNAL_MESSAGE = 'This mode is planned or internal and is not part of the supported public workflow.';
...
async function findBuiltStoryArtifact(outDir, preferred) {
  if (preferred && await pathExists(preferred)) return path.resolve(preferred);
  const dir = normalizeString(outDir);
  if (!dir) return null;
  return await firstExistingPath([
    path.join(dir, 'story.mvsx'),
    path.join(dir, 'index.mvsx'),
    path.join(dir, 'docking_story.mvsx'),
    path.join(dir, 'story.mvsj'),
    path.join(dir, 'index.mvsj'),
    path.join(dir, 'story.html'),
    path.join(dir, 'index.html'),
  ]);
}

function wantsOpen(input = {}) {
  return explicitTrue(input.open) && !explicitTrue(input.no_open) && !explicitTrue(input.noOpen);
}

function wantsExternalBrowserOpen(input = {}) {
  return (explicitTrue(input.openBrowser) || explicitTrue(input.open_browser))
    && !explicitTrue(input.noOpenBrowser)
    && !explicitTrue(input.no_open_browser);
}

function wantsCodexPreview(input = {}) {
  return (explicitTrue(input.codexPreview)
      || explicitTrue(input.codex_preview)
      || explicitTrue(input.openCodex)
      || explicitTrue(input.open_codex))
    && !explicitTrue(input.noCodexPreview)
    && !explicitTrue(input.no_codex_preview);
}

function wantsServe(input = {}) {
  return explicitTrue(input.serve) || wantsOpen(input) || wantsCodexPreview(input) || wantsExternalBrowserOpen(input);
}

function warningList(...items) {
  const warnings = items.flatMap(item => {
    if (!item) return [];
    if (Array.isArray(item)) return item.filter(Boolean).map(String);
    return [String(item)];
  });
...
function summarizeRunResult(result) {
  ...
  return {
    ...result,
    stdout: stdout.length <= 20000 ? stdout : undefined,
    stderr: stderr.length <= 20000 ? stderr : undefined,
    stdout_bytes: stdout ? Buffer.byteLength(stdout, 'utf8') : 0,
    stderr_bytes: stderr ? Buffer.byteLength(stderr, 'utf8') : 0,
    stdout_preview: stdout.length > 20000 ? stdout.slice(0, 2000) : undefined,
    stderr_preview: stderr.length > 20000 ? stderr.slice(0, 2000) : undefined,
  };
}

function deprecatedOpenWarnings(input = {}) {
  const warnings = [];
  if (wantsOpen(input)) {
    warnings.push('--open is deprecated/ambiguous and now requests serving; use --codex-preview for Codex preview, or --open-browser for system browser.');
  }
  if (explicitTrue(input.openCodex) || explicitTrue(input.open_codex)) {
    warnings.push('--open-codex is deprecated; use --codex-preview for Codex preview metadata and in-app Browser navigation. It never opens the system browser.');
  }
  return warnings;
}

function codexPreviewInstructions(viewerUrl) {
  const url = normalizeString(viewerUrl);
  if (!url) return null;
  return {
    hostRequirement: 'Codex Browser plugin or equivalent preview/navigation capability',
    action: 'show-in-app-browser-and-navigate',
    browserId: 'iab',
    viewerUrl: url,
    preferredTabBehavior: 'reuse-current-tab',
    steps: [
      'Use browser:control-in-app-browser when available.',
      'Set browser visibility to true.',
      'Reuse the selected in-app tab when one is already open; create one only if no in-app Browser tab exists.',
      'Navigate the in-app tab to viewerUrl.',
      'Verify the tab URL/title and window.__MVS_LOAD_STATUS when possible.',
      'Do not use --open-browser or the system/default browser unless the user explicitly asks for it.',
    ],
  };
}

function codexPreviewHubInstructions(hubUrl) {
  const url = normalizeString(hubUrl);
  if (!url) return null;
  return {
    hostRequirement: 'Codex Browser plugin or equivalent preview/navigation capability',
    action: 'show-in-app-browser-and-navigate-same-tab',
    browserId: 'iab',
    hubUrl: url,
    preferredBehavior: 'open-hub-in-current-tab',
    openOnly: 'hubUrl',
    doNotOpenEachViewerUrlSeparately: true,
    steps: [
      'Use browser:control-in-app-browser when available.',
      'Set browser visibility to true.',
      'Reuse the selected in-app tab when one is already open; create one only if no in-app Browser tab exists.',
      'Navigate that tab to hubUrl only.',
      'Do not open viewerUrls individually as separate in-app Browser tabs.',
      'Switch stories inside the hub page using its buttons/tabs.',
      'Do not use --open-browser or the system/default browser unless the user explicitly asks for it.',
    ],
  };
}

function portFromViewerUrl(url) {
...
function normalizeDisplayIntent(input = {}) {
  const display = normalizeString(input.display || input.display_intent || input.displayIntent || 'codex-preview').toLowerCase();
  if (display === 'url-only' || display === 'url_only') return 'url-only';
  if (display === 'external-browser' || display === 'external_browser') return 'external-browser';
  if (wantsCodexPreview(input)) return 'open-in-codex-preview';
  return 'open-in-codex-preview';
}

function viewerResponseMetadata(input = {}, fields = {}) {
  const viewerUrl = normalizeString(fields.viewerUrl || fields.viewer_url);
  const artifact = normalizeString(fields.artifact);
  const port = fields.port || portFromViewerUrl(viewerUrl);
  const served = Boolean(fields.served && viewerUrl);
  const displayIntent = normalizeDisplayIntent(input);
  const externalBrowserOpenRequested = Boolean(fields.externalBrowserOpenRequested);
  const warnings = warningList(fields.warnings);
  return {
    kind: 'molstar-viewer',
    viewerType: normalizeString(fields.viewerType || fields.viewer || 'molstory') || 'molstory',
    displayIntent,
    viewerUrl: viewerUrl || null,
    artifact: artifact || null,
    served,
    port,
    externalBrowserOpenRequested,
    codexPreviewRequested: wantsCodexPreview(input) || displayIntent === 'open-in-codex-preview',
    warnings,
    codexPreview: codexPreviewInstructions(viewerUrl),
    previewNote: served && displayIntent === 'open-in-codex-preview'
      ? 'Viewer is served, but automatic opening depends on whether the Codex host exposes preview/navigation capability.'
      : undefined,
  };
}

function attachViewerResponse(result, input = {}, fields = {}) {
  const metadata = viewerResponseMetadata(input, {
    ...fields,
    viewerUrl: fields.viewerUrl || result?.viewerUrl || result?.server?.url,
    artifact: fields.artifact || result?.artifact || result?.server?.artifact || result?.server?.input || result?.input,
    port: fields.port || result?.port || result?.server?.port,
    served: fields.served ?? result?.served ?? Boolean(result?.server?.url),
    externalBrowserOpenRequested: fields.externalBrowserOpenRequested ?? result?.externalBrowserOpenRequested,
    warnings: fields.warnings || result?.warnings,
  });
  return {
    ...result,
    ...metadata,
    viewerResource: {
      type: 'link',
      uri: metadata.viewerUrl,
      mimeType: 'text/html',
      title: 'Mol* Stories viewer',
      displayIntent: metadata.displayIntent,
    },
  };
}
...
function parseHubInputSpec(raw, index) {
  const spec = normalizeString(raw);
  if (!spec) throw new Error('empty --inputs value is not allowed');
  const eq = spec.indexOf('=');
  const label = eq > 0 ? spec.slice(0, eq).trim() : '';
  const file = eq > 0 ? spec.slice(eq + 1).trim() : spec;
  if (!file) throw new Error(`missing artifact path in --inputs value: ${spec}`);
  const artifact = path.resolve(file);
  return {
    label: label || stem(artifact) || `Story ${index + 1}`,
    artifact,
  };
}

function parseHubInputs(input = {}) {
  const rawInputs = [
    ...normalizeRepeated(input.inputs),
    ...normalizeRepeated(input.input),
  ];
  if (!rawInputs.length) throw new Error('at least one --inputs "Label=<story>" value is required');
  return rawInputs.map((item, index) => parseHubInputSpec(item, index));
}

function hubHtml({ title, viewers }) {
  const first = viewers[0] || {};
  const viewerJson = JSON.stringify(viewers.map(item => ({
    label: item.label,
...
<head>
  ...
  <style>
    :root { color-scheme: light; --bg: #f6f5f1; --panel: #ffffff; --line: #d9d6cf; --text: #15171a; --muted: #60656d; --active: #234f9c; }
    html, body { margin: 0; height: 100%; overflow: hidden; font-family: system-ui, -apple-system, Segoe UI, sans-serif; background: var(--bg); color: var(--text); }
    ...
    h1 { font-size: 15px; margin: 0 10px 0 0; font-weight: 650; white-space: nowrap; }
    .tabs { display: flex; gap: 6px; overflow-x: auto; min-width: 0; }
    button { border: 1px solid var(--line); border-radius: 6px; background: #f8f8f6; color: var(--text); padding: 7px 11px; font: inherit; cursor: pointer; white-space: nowrap; }
    button[aria-selected="true"] { background: var(--active); color: white; border-color: var(--active); }
    .meta { margin-left: auto; color: var(--muted); font-size: 12px; white-space: nowrap; }
    iframe { width: 100%; height: 100%; border: 0; background: white; }
    #status { position: fixed; right: 12px; bottom: 12px; max-width: min(680px, calc(100% - 24px)); padding: 8px 10px; border-radius: 6px; background: rgba(0,0,0,.72); color: #fff; font: 12px/1.4 system-ui, sans-serif; z-index: 20; }
...
<body>
  <main class="shell">
    <header>
      ...
      <nav class="tabs" id="tabs" aria-label="Story selector"></nav>
      <div class="meta">One Codex tab, multiple stories</div>
    </header>
    <iframe id="viewer" title="${htmlEscape(first.label || 'Mol* story')}" src="${htmlEscape(first.viewerUrl || 'about:blank')}" allow="fullscreen"></iframe>
  </main>
  <div id="status">Loading ${htmlEscape(first.label || 'story')}...</div>
  <script>
    const viewers = ${viewerJson};
    const tabs = document.getElementById('tabs');
    const iframe = document.getElementById('viewer');
    const statusBox = document.getElementById('status');
    window.__LP_FLOW_VIEWER_HUB = { loaded: true, selected: null, viewers };
    function selectViewer(index) {
      const item = viewers[index];
      if (!item) return;
      iframe.src = item.viewerUrl;
      iframe.title = item.label + ' Mol* Story';
      window.__LP_FLOW_VIEWER_HUB.selected = item;
      document.documentElement.dataset.selectedStory = item.label;
      statusBox.textContent = 'Showing ' + item.label;
    ...
    viewers.forEach((item, index) => {
      ...
      button.addEventListener('click', () => selectViewer(index));
...
function hubResponse(input = {}, fields = {}) {
  const hubUrl = normalizeString(fields.hubUrl || fields.hub_url);
  const port = fields.port || portFromViewerUrl(hubUrl);
  const displayIntent = normalizeDisplayIntent(input);
  const warnings = warningList(fields.warnings);
  return {
    ok: Boolean(fields.ok !== false && hubUrl),
    kind: 'molstar-viewer-hub',
    displayIntent,
    hubUrl: hubUrl || null,
    viewerUrls: fields.viewerUrls || [],
    hubDir: fields.hubDir || null,
    served: Boolean(fields.served && hubUrl),
    port,
    externalBrowserOpenRequested: false,
    warnings,
    codexPreviewRequested: wantsCodexPreview(input) || displayIntent === 'open-in-codex-preview',
    codexPreview: codexPreviewHubInstructions(hubUrl),
    viewerResource: {
      type: 'link',
      uri: hubUrl,
      mimeType: 'text/html',
      title: 'Mol* Stories preview hub',
      displayIntent,
    },
    previewNote: hubUrl && displayIntent === 'open-in-codex-preview'
      ? 'Hub is served for one Codex Browser tab. Open hubUrl only; switch stories inside the hub.'
      : undefined,
  };
}
...
function summarizeExecution(execution) {
  ...
  return {
    ...execution,
    result: summarizeRunResult(execution.result),
  };
}

async function detectMvsBuildCommand(input = {}, storyDir, outputFile) {
  const explicitMvs = normalizeString(input.mvs || input.mvsPath || input.mvs_cli || input.mvsCli || getEnv('LP_FLOW_MVS'));
  if (explicitMvs) {
    return {
      ok: true,
      tool: 'mvs',
      argv: [explicitMvs, 'build', storyDir, '--format', 'mvsx', '-o', outputFile],
      source: 'explicit mvs command',
    };
  }

  const mvsProbe = await runCommand(['mvs', '--help'], { timeout_ms: 10000, max_output_bytes: 20000 });
  if (mvsProbe.ok) {
    return {
      ok: true,
      tool: 'mvs',
      argv: ['mvs', 'build', storyDir, '--format', 'mvsx', '-o', outputFile],
      source: 'mvs on PATH',
    };
  }

  const mvsMain = normalizeString(
    input.mvs_main
    || input.mvsMain
    || input.mvs_cli_main
    || input.mvsCliMain
    || getEnv('LP_FLOW_MVS_CLI_MAIN')
  );
  if (mvsMain) {
    const resolvedMain = path.resolve(mvsMain);
    if (!(await pathExists(resolvedMain))) {
      return {
        ok: false,
        error: `Configured Mol View Stories CLI entry point does not exist: ${resolvedMain}`,
        mvs_probe: summarizeRunResult(mvsProbe),
      };
    }
    const deno = await detectDeno(input);
    if (!deno.ok) {
      return {
        ok: false,
        error: 'Deno runtime unavailable; cannot run configured Mol View Stories CLI entry point.',
        deno,
        mvs_probe: summarizeRunResult(mvsProbe),
      };
    }
    return {
      ok: true,
      tool: 'deno-mvs-cli',
      argv: [deno.deno, 'run', '-A', resolvedMain, 'build', storyDir, '--format', 'mvsx', '-o', outputFile],
      source: 'configured Mol View Stories CLI entry point',
      deno,
    };
  }
  ...
  return {
    ok: false,
    error: 'No Mol View Stories build command was found. Install/provide mvs, set LP_FLOW_MVS, or set LP_FLOW_MVS_CLI_MAIN to a mol-view-stories cli/main.ts path.',
    mvs_probe: summarizeRunResult(mvsProbe),
  };
}

async function buildDockingStoryArtifactForViewer(input, outDir, currentArtifact) {
  if (currentArtifact) {
    return {
      ok: true,
      skipped: true,
      artifact: path.resolve(currentArtifact),
      reason: 'Built story artifact already exists.',
    };
  }
  const storyDir = path.resolve(outDir);
  const storyYaml = path.join(storyDir, 'story.yaml');
  if (!(await pathExists(storyYaml))) {
    return {
      ok: false,
      error: `Docking story source was not found at ${storyYaml}; cannot build viewer-ready artifact.`,
    };
  }
  const artifact = path.join(storyDir, 'docking_story.mvsx');
  const command = await detectMvsBuildCommand(input, storyDir, artifact);
  if (!command.ok) {
    return {
      ok: false,
      artifact,
  ...
  const result = await runCommand(command.argv, { timeout_ms: input.timeout_ms || input.timeoutMs || 180000, max_output_bytes: 100000 });
  const existsAfter = await pathExists(artifact);
  return {
    ok: result.ok && existsAfter,
    artifact: existsAfter ? artifact : null,
    command: {
      ...command,
      command_line: commandLineFromArgv(command.argv),
    },
    result: summarizeRunResult(result),
    error: result.ok && !existsAfter ? `Mol View Stories build completed but did not create ${artifact}` : undefined,
  };
}

async function openViewerUrl(url) {
  ...
  if (!targetUrl) {
    return {
      ...
      attempted: false,
      succeeded: false,
      error: 'No viewer URL was available to open.',
    };
  }
  let argv;
  const platform = currentPlatform();
  if (platform === 'win32') {
    argv = ['cmd', '/c', 'start', '', targetUrl];
  } else if (platform === 'darwin') {
    argv = ['open', targetUrl];
  } else {
    argv = ['xdg-open', targetUrl];
  }
  const result = await runCommand(argv, { timeout_ms: 10000, max_output_bytes: 20000 });
  return {
    requested: true,
    attempted: true,
    succeeded: result.ok,
    ...
    result: summarizeRunResult(result),
    error: result.ok ? undefined : 'Browser open command failed; open viewerUrl manually.',
  };
}

async function detectDeno(input = {}) {
  const explicit = normalizeString(input.deno || input.denoPath || getEnv('DENO') || getEnv('DENO_EXE'));
  const home = os.homedir ? os.homedir() : '';
  const candidates = explicit
    ? [explicit]
    : [
        home ? path.join(home, '.codex', 'tools', 'deno', currentPlatform() === 'win32' ? 'deno.exe' : 'deno') : '',
        home ? path.join(home, '.deno', 'bin', currentPlatform() === 'win32' ? 'deno.exe' : 'deno') : '',
        'deno',
      ].filter(Boolean);
  for (const candidate of candidates) {
    if (candidate !== 'deno' && !(await pathExists(candidate))) continue;
    const probe = await runCommand([candidate, '--version'], { timeout_ms: 10000, max_output_bytes: 20000 });
    if (probe.ok) {
      return {
        ok: true,
        deno: candidate,
  ...
  return {
    ...
    deno: null,
    status: 'skipped',
    reason: 'Deno runtime unavailable',
  };
}

async function extractMvsxForValidation(inputPath) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'lp-flow-mvsx-'));
  const result = await runCommand(['tar', '-xf', inputPath, '-C', tempDir], { timeout_ms: 60000, max_output_bytes: 50000 });
  if (!result.ok) {
    return {
      ok: false,
      temp_dir: tempDir,
      error: 'Unable to extract MVSX archive with tar.',
      result,
    };
  }
  const mvsj = path.join(tempDir, 'index.mvsj');
  if (!(await pathExists(mvsj))) {
    return {
      ok: false,
      temp_dir: tempDir,
      error: 'MVSX archive did not contain index.mvsj.',
    };
  }
  return { ok: true, temp_dir: tempDir, mvsj };
}

async function validateStory(input) {
  const rawInput = normalizeString(input.input || input.story || input.file);
  if (!rawInput) throw new Error('input is required');
  const storyInput = path.resolve(rawInput);
  if (!(await pathExists(storyInput))) {
    return {
      ok: false,
      input: storyInput,
  ...
  const suffix = path.extname(storyInput).toLowerCase();
  if (!['.mvsj', '.mvsx'].includes(suffix)) {
    return unsupportedCombination('story validate currently supports .mvsj and .mvsx artifacts only.', {
      input: storyInput,
      supported_extensions: ['.mvsj', '.mvsx'],
    });
  }
  const deno = await detectDeno(input);
  if (!deno.ok) {
    return {
      ...
      note: 'Official Mol* validation is optional. Do not run validate_mvsj_official.mjs with Node.',
    };
  }
  let validationInput = storyInput;
  let extraction = null;
  if (suffix === '.mvsx') {
    extraction = await extractMvsxForValidation(storyInput);
    if (!extraction.ok) {
      return {
        ok: false,
        input: storyInput,
    ...
    validationInput = extraction.mvsj;
  }
  const script = path.join(SERVER_DOCKING_SCRIPTS, 'validate_mvsj_official.mjs');
  const argv = [deno.deno, 'run', '--allow-read', script, validationInput];
  const result = await runCommand(argv, { timeout_ms: input.timeout_ms || input.timeoutMs || 120000 });
  ...
  return {
    ...
    deno,
    extraction: extraction ? { temp_dir: extraction.temp_dir, mvsj: extraction.mvsj } : null,
    command: {
      argv,
      command_line: commandLineFromArgv(argv),
    },
    official_validation: parsed || {
      status: result.ok ? 'completed_unparsed' : 'failed',
...
async function startJsonServerCommand(argv, options = {}) {
  const { spawn } = await import('node:child_process');
  const timeoutMs = Number.parseInt(options.timeout_ms || options.timeoutMs || 15000, 10);
  if (options.expected_url) {
    const launch = await startPersistentLocalProcess(argv);
    if (!launch.ok) {
      return {
        ok: false,
        status: 'failed_to_start',
        command: { argv, command_line: commandLineFromArgv(argv) },
        launcher: launch,
        server: {
          url: options.expected_url,
          host: options.expected_host || null,
          port: options.expected_port || portFromViewerUrl(options.expected_url),
          input: options.expected_input || null,
          artifact: options.expected_artifact || options.expected_input || null,
        },
      };
    }
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
      if (await httpReady(options.expected_url)) {
        const keepAlive = setTimeout(() => {}, 1000);
        setTimeout(() => clearTimeout(keepAlive), 250);
        return {
          ok: true,
          pid: launch.pid || null,
          command: { argv, command_line: commandLineFromArgv(argv) },
    ...
    await terminateProcessByPid(launch.pid);
    const keepAlive = setTimeout(() => {}, 1000);
    setTimeout(() => clearTimeout(keepAlive), 250);
    return {
      ok: false,
...
async function startPersistentLocalProcess(argv) {
  const { spawn } = await import('node:child_process');
  if (currentPlatform() === 'win32') {
    const psArgs = argv.slice(1).map(arg => psSingleQuote(arg)).join(', ');
    const script = `$ErrorActionPreference = 'Stop'\n$ProgressPreference = 'SilentlyContinue'\n$argList = @(${psArgs})\n$p = Start-Process -WindowStyle Hidden -FilePath ${psSingleQuote(argv[0])} -ArgumentList $argList -PassThru\n$p.Id`;
  ...
  return {
    ok: true,
    pid: child.pid,
    launcher: 'detached child process',
  };
}

async function preferredLocalPort(host, startPort) {
  const net = await import('node:net');
  const firstPort = Number.isFinite(startPort) && startPort > 0 ? startPort : 8765;
...
async function serveStory(input) {
  const rawInput = normalizeString(input.input || input.story || input.file);
  if (!rawInput) throw new Error('input is required');
  const storyInput = path.resolve(rawInput);
  const node = normalizeString(input.node) || (typeof process !== 'undefined' && process.execPath ? process.execPath : 'node');
  const script = path.join(SERVER_DOCKING_SCRIPTS, 'serve_mvs_story.mjs');
  const host = normalizeString(input.host) || '127.0.0.1';
  const selectedPort = await preferredLocalPort(host, Number.parseInt(input.port || '8765', 10));
  const expectedUrl = `http://${host}:${selectedPort}/index.html`;
  const argv = [node, script, '--input', storyInput];
  argv.push('--port', String(selectedPort));
  argv.push('--host', host);
  if (input.title) argv.push('--title', String(input.title));
  if (input.asset_dir || input.assetDir) argv.push('--asset-dir', String(input.asset_dir || input.assetDir));
  const served = await startJsonServerCommand(argv, {
    ...input,
    expected_url: expectedUrl,
    expected_host: host,
    expected_port: selectedPort,
    expected_input: storyInput,
    expected_artifact: storyInput,
  });
  const warnings = warningList(deprecatedOpenWarnings(input));
  let externalBrowserOpen = {
    requested: wantsExternalBrowserOpen(input),
    attempted: false,
    succeeded: false,
  };
  const viewerUrl = served?.server?.url || null;
  const port = served?.server?.port || portFromViewerUrl(viewerUrl);
  if (served.ok && wantsExternalBrowserOpen(input)) {
    externalBrowserOpen = await openViewerUrl(viewerUrl);
    if (!externalBrowserOpen.succeeded) warnings.push('Browser open command failed; open viewerUrl manually.');
  }
  return attachViewerResponse({
    ...served,
    input: storyInput,
    artifact: served?.server?.artifact || served?.server?.input || storyInput,
    viewerUrl,
    serverStatus: served?.ok && viewerUrl ? 'running' : served?.status || 'failed',
    port,
    served: Boolean(served.ok && viewerUrl),
    externalBrowserOpenRequested: externalBrowserOpen.requested,
    externalBrowserOpenAttempted: externalBrowserOpen.attempted,
    externalBrowserOpenSucceeded: externalBrowserOpen.succeeded,
    openRequested: wantsOpen(input) || externalBrowserOpen.requested,
    openAttempted: externalBrowserOpen.attempted,
    openSucceeded: externalBrowserOpen.succeeded,
    open: {
      ...externalBrowserOpen,
      deprecatedAliasRequested: wantsOpen(input),
    },
    warnings,
  }, input, {
    viewerType: normalizeString(input.viewer || input.viewerType || 'molstory'),
    viewerUrl,
    artifact: served?.server?.artifact || served?.server?.input || storyInput,
    port,
    served: Boolean(served.ok && viewerUrl),
...
async function serveMdTrajectory(input = {}) {
  const structureRaw = normalizeString(input.structure || input.topology || input.reference || input.gro || input.pdb);
  const trajectoryRaw = normalizeString(input.trajectory || input.xtc || input.trr || input.dcd);
  if (!structureRaw) throw new Error('structure is required; pass --structure <reference.gro|pdb|cif>');
  if (!trajectoryRaw) throw new Error('trajectory is required; pass --trajectory <trajectory.xtc|trr|dcd>');
  const structure = path.resolve(structureRaw);
  const trajectory = path.resolve(trajectoryRaw);
  if (!(await pathExists(structure))) throw new Error(`structure does not exist: ${structure}`);
  if (!(await pathExists(trajectory))) throw new Error(`trajectory does not exist: ${trajectory}`);
  const node = normalizeString(input.node) || (typeof process !== 'undefined' && process.execPath ? process.execPath : 'node');
  const script = path.join(PLUGIN_ROOT, 'scripts', 'gromacs-md', 'serve_md_trajectory.mjs');
  const host = normalizeString(input.host) || '127.0.0.1';
  const selectedPort = await preferredLocalPort(host, Number.parseInt(input.port || '8890', 10));
  const expectedUrl = `http://${host}:${selectedPort}/index.html`;
  const title = normalizeString(input.title) || 'LP-Flow MD trajectory';
  const argv = [
    node,
    script,
    '--structure',
  ...
  if (input.ngl_url || input.nglUrl) argv.push('--ngl-url', String(input.ngl_url || input.nglUrl));
  const served = await startJsonServerCommand(argv, {
    ...input,
    expected_url: expectedUrl,
    expected_host: host,
    expected_port: selectedPort,
    expected_input: trajectory,
    expected_artifact: trajectory,
  });
  const warnings = warningList(deprecatedOpenWarnings(input));
  let externalBrowserOpen = {
    requested: wantsExternalBrowserOpen(input),
  ...
  const viewerUrl = served?.server?.url || served?.viewerUrl || null;
  const port = served?.server?.port || served?.port || portFromViewerUrl(viewerUrl);
  if (served.ok && wantsExternalBrowserOpen(input)) {
    externalBrowserOpen = await openViewerUrl(viewerUrl);
    if (!externalBrowserOpen.succeeded) warnings.push('Browser open command failed; open viewerUrl manually.');
  }
  return attachViewerResponse({
    ...served,
    ok: Boolean(served.ok && viewerUrl),
    ...
    warnings,
    note: 'Legacy/local native trajectory viewer: loads structure/topology and trajectory as separate files. For Burrete workflow completion, use a bounded no-water multi-frame PDB display artifact and keep native files for interpretation/provenance.',
  }, input, {
...
async function serveStoryHub(input) {
  const entries = parseHubInputs(input);
  const host = normalizeString(input.host) || '127.0.0.1';
  const node = normalizeString(input.node) || (typeof process !== 'undefined' && process.execPath ? process.execPath : 'node');
  const basePort = Number.parseInt(input.port || '8765', 10);
  const outDirRaw = normalizeString(input.outDir || input.out_dir);
  const hubDir = outDirRaw
    ? path.resolve(outDirRaw)
    : await fs.mkdtemp(path.join(os.tmpdir(), 'lp-flow-story-hub-'));
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    const child = await serveStory({
      input: entry.artifact,
      title: entry.label,
      host,
    ...
    if (!child.ok || !child.viewerUrl) {
      warnings.push(`Viewer failed for ${entry.label}: ${child.serverStatus || child.reason || 'unknown error'}`);
      continue;
    }
    viewerUrls.push({
      label: entry.label,
      viewerUrl: child.viewerUrl,
      artifact: child.artifact || entry.artifact,
      port: child.port || portFromViewerUrl(child.viewerUrl),
    });
  }
  if (!viewerUrls.length) {
    return hubResponse(input, {
      ok: false,
      hubUrl: null,
      hubDir,
      served: false,
      viewerUrls,
      warnings: warningList(warnings, 'No child story viewers could be served.'),
    });
  }

  const title = normalizeString(input.title) || 'LP-Flow Mol* Stories Hub';
  const hubFile = path.join(hubDir, 'index.html');
  await fs.writeFile(hubFile, hubHtml({ title, viewers: viewerUrls }), 'utf8');
  const hubServer = await serveStory({
    input: hubDir,
    title,
    host,
    port: String(basePort + entries.length),
    node,
    display: normalizeDisplayIntent(input),
    timeout_ms: input.timeout_ms || input.timeoutMs,
  });
  const hubUrl = hubServer.viewerUrl || hubServer.server?.url || null;
  const response = hubResponse(input, {
    ok: Boolean(hubServer.ok && hubUrl),
    hubUrl,
    hubDir,
    port: hubServer.port || portFromViewerUrl(hubUrl),
    served: Boolean(hubServer.ok && hubUrl),
    viewerUrls,
    warnings: warningList(warnings, hubServer.warnings),
  });
  return {
    ...response,
    hubFile,
    childServers: childServers.map(item => ({
      ok: Boolean(item.ok),
      viewerUrl: item.viewerUrl,
      artifact: item.artifact,
      port: item.port,
    ...
    hubServer: {
      ok: Boolean(hubServer.ok),
      viewerUrl: hubUrl,
      port: hubServer.port || portFromViewerUrl(hubUrl),
      serverStatus: hubServer.serverStatus,
    },
  };
}

async function renderStoryPackage(input) {
  const rawInput = normalizeString(input.input || input.story || input.file);
  if (!rawInput) throw new Error('input is required');
  const storyInput = path.resolve(rawInput);
  if (!(await pathExists(storyInput))) throw new Error(`input story does not exist: ${storyInput}`);
  const outDirRaw = normalizeString(input.outDir || input.out_dir);
  if (!outDirRaw) throw new Error('out-dir is required for story render-package');
  const outDir = path.resolve(outDirRaw);
  const seconds = Number.parseFloat(input.seconds || '30');
  const warmupSeconds = Number.parseFloat(input.warmup_seconds || input.warmupSeconds || '20');
  const sceneSeconds = Number.parseFloat(input.scene_seconds || input.sceneSeconds || '5');
  const playbackMode = normalizeString(input.playback_mode || input.playbackMode) || 'story';
  const requireHardwareWebgl = explicitTrue(input.require_hardware_webgl || input.requireHardwareWebgl);
  const width = Number.parseInt(input.width || '1920', 10);
  const height = Number.parseInt(input.height || '1080', 10);
  const glMode = normalizeString(input.gl) || 'swiftshader';
  if (!Number.isFinite(seconds) || seconds <= 0) throw new Error('seconds must be a positive number');
  if (!Number.isFinite(warmupSeconds) || warmupSeconds < 0) throw new Error('warmup-seconds must be zero or a positive number');
  if (!Number.isFinite(sceneSeconds) || sceneSeconds <= 0) throw new Error('scene-seconds must be a positive number');
  if (!Number.isFinite(width) || width < 320) throw new Error('width must be at least 320');
  if (!Number.isFinite(height) || height < 240) throw new Error('height must be at least 240');
  if (!['swiftshader', 'egl', 'default'].includes(glMode)) throw new Error('gl must be swiftshader, egl, or default');
  if (!['story', 'cinematic'].includes(playbackMode)) throw new Error('playback-mode must be story or cinematic');

  const packageDir = path.join(outDir, 'story_render_package');
  await fs.mkdir(packageDir, { recursive: true });
  const storyName = path.basename(storyInput);
  const packagedStory = path.join(packageDir, storyName);
  await fs.copyFile(storyInput, packagedStory);
  const dependencies = await copyStoryDependencies(storyInput, packageDir);
  const renderScriptSource = path.join(SERVER_DOCKING_SCRIPTS, 'render_story_video.mjs');
  const renderScript = path.join(packageDir, 'render_story_video.mjs');
  await fs.copyFile(renderScriptSource, renderScript);
  const serveScriptSource = path.join(SERVER_DOCKING_SCRIPTS, 'serve_mvs_story.mjs');
  const serveScript = path.join(packageDir, 'serve_mvs_story.mjs');
  await fs.copyFile(serveScriptSource, serveScript);
  const assetSourceDir = path.join(PLUGIN_ASSETS, 'mvs-stories', '5.8.0');
  const assetPackageDir = path.join(packageDir, 'mvs-assets');
  await fs.mkdir(assetPackageDir, { recursive: true });
  for (const name of ['mvs-stories.js', 'mvs-stories.css']) {
    await fs.copyFile(path.join(assetSourceDir, name), path.join(assetPackageDir, name));
  }

  const renderOutDir = 'render_out';
  const shellScript = `#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"
NODE_BIN="\${NODE_BIN:-node}"
PLAYWRIGHT_PKG="\${PLAYWRIGHT_PKG:-playwright}"
FFMPEG_BIN="\${FFMPEG_BIN:-ffmpeg}"
GL_MODE="\${GL_MODE:-${glMode}}"
"$NODE_BIN" render_story_video.mjs \\
  --input ${posixQuote(storyName)} \\
  --out-dir ${posixQuote(renderOutDir)} \\
  --width ${width} \\
  --height ${height} \\
  --seconds ${seconds} \\
  --warmup-seconds ${warmupSeconds} \\
  --scene-seconds ${sceneSeconds} \\
  --playback-mode ${posixQuote(playbackMode)} \\
  --require-hardware-webgl ${requireHardwareWebgl ? 'true' : 'false'} \\
  --gl "$GL_MODE" \\
  --playwright "$PLAYWRIGHT_PKG" \\
  --ffmpeg "$FFMPEG_BIN"
tar -czf render_results.tar.gz "${renderOutDir}"
`;
  await fs.writeFile(path.join(packageDir, 'run_render.sh'), shellScript, 'utf8');
  const psScript = `$ErrorActionPreference = 'Stop'
$NodeBin = if ($env:NODE_BIN) { $env:NODE_BIN } else { 'node' }
$PlaywrightPkg = if ($env:PLAYWRIGHT_PKG) { $env:PLAYWRIGHT_PKG } else { 'playwright' }
$FfmpegBin = if ($env:FFMPEG_BIN) { $env:FFMPEG_BIN } else { 'ffmpeg' }
$GlMode = if ($env:GL_MODE) { $env:GL_MODE } else { ${JSON.stringify(glMode)} }
Set-Location -LiteralPath $PSScriptRoot
& $NodeBin .\\render_story_video.mjs --input ${JSON.stringify(storyName)} --out-dir ${JSON.stringify(renderOutDir)} --width ${width} --height ${height} --seconds ${seconds} --warmup-seconds ${warmupSeconds} --scene-seconds ${sceneSeconds} --playback-mode ${JSON.stringify(playbackMode)} --require-hardware-webgl ${requireHardwareWebgl ? 'true' : 'false'} --gl $GlMode --playwright $PlaywrightPkg --ffmpeg $FfmpegBin
tar -czf render_results.tar.gz ${JSON.stringify(renderOutDir)}
`;
  await fs.writeFile(path.join(packageDir, 'run_render.ps1'), psScript, 'utf8');

  const manifest = {
    ok: true,
    kind: 'story-render-package',
    workflow: 'story render-package',
    created_at: new Date().toISOString(),
    package_dir: packageDir,
    input_story: storyInput,
    packaged_story: packagedStory,
    dependencies,
    render_script: renderScript,
    serve_script: serveScript,
    viewer_assets: assetPackageDir,
    run_scripts: {
      bash: path.join(packageDir, 'run_render.sh'),
      powershell: path.join(packageDir, 'run_render.ps1'),
    },
    expected_outputs: {
      render_manifest: path.join(packageDir, renderOutDir, 'render_manifest.json'),
      webm: path.join(packageDir, renderOutDir, 'story_render.webm'),
      mp4: path.join(packageDir, renderOutDir, 'story_render.mp4'),
      archive: path.join(packageDir, 'render_results.tar.gz'),
    },
    render_options: {
      width,
      height,
      ...
      require_hardware_webgl: requireHardwareWebgl,
      auto_step: playbackMode === 'story',
      gl_mode: glMode,
      playwright_package: normalizeString(input.playwright) || 'playwright',
      ffmpeg: normalizeString(input.ffmpeg) || 'ffmpeg',
    },
    server_notes: [
      'Install/provide Node, Playwright with Chromium, and ffmpeg on the render host.',
      'For GPU/EGL rendering, ensure headless Chromium can access EGL/WebGL; otherwise Chromium may fall back or fail.',
      'Run ./run_render.sh on Linux server, then download render_results.tar.gz.',
    ],
  };
  await fs.writeFile(path.join(packageDir, 'render_package_manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
  if (explicitTrue(input.archive) || explicitTrue(input.package)) {
    const packageArchive = path.join(outDir, 'story_render_package.tar.gz');
    const tar = await runCommand(['tar', '-czf', packageArchive, '-C', outDir, 'story_render_package'], {
      timeout_ms: Number.parseInt(input.timeout_ms || '60000', 10),
      max_output_bytes: 20000,
    });
    manifest.package_archive = packageArchive;
    manifest.archive_result = summarizeRunResult(tar);
    await fs.writeFile(path.join(packageDir, 'render_package_manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
  }
  return manifest;
}

function collectMvsjDependencyUrls(value, output = new Set()) {
  if (!value || typeof value !== 'object') return output;
  if (Array.isArray(value)) {
    for (const item of value) collectMvsjDependencyUrls(item, output);
    return output;
  }
  if (value.kind === 'download' && value.params && typeof value.params.url === 'string') {
    output.add(value.params.url);
  }
  for (const child of Object.values(value)) collectMvsjDependencyUrls(child, output);
  return output;
}

function isLocalStoryDependency(url) {
  const raw = normalizeString(url);
  if (!raw) return false;
...
async function copyStoryDependencies(storyInput, packageDir) {
  if (path.extname(storyInput).toLowerCase() !== '.mvsj') return [];
  const storyDir = path.dirname(storyInput);
  let parsed;
  try {
    parsed = JSON.parse(await fs.readFile(storyInput, 'utf8'));
  } catch {
    return [];
  }
  const copied = [];
  for (const url of collectMvsjDependencyUrls(parsed)) {
    if (!isLocalStoryDependency(url)) continue;
    const cleanRel = url.split(/[?#]/, 1)[0].replace(/\\/g, '/');
    if (!cleanRel || cleanRel.startsWith('/') || cleanRel.split('/').includes('..')) continue;
    const source = path.resolve(storyDir, cleanRel);
    const storyDirResolved = path.resolve(storyDir);
    if (!(source === storyDirResolved || source.startsWith(`${storyDirResolved}${path.sep}`))) continue;
    if (!(await pathExists(source))) continue;
    const target = path.join(packageDir, ...cleanRel.split('/'));
...
async function maybeServeOrSkip(input, artifact) {
  if (!wantsServe(input)) return null;
  if (!artifact) {
    return {
      ok: false,
      status: 'failed',
      reason: 'No built .mvsj/.mvsx/.html artifact was found. Build/export the story source first, then run story serve.',
      serverStatus: 'failed',
      port: null,
      externalBrowserOpenRequested: wantsExternalBrowserOpen(input),
      externalBrowserOpenAttempted: false,
      externalBrowserOpenSucceeded: false,
      openRequested: wantsOpen(input) || wantsExternalBrowserOpen(input),
      openAttempted: false,
      openSucceeded: false,
      warnings: warningList(
        deprecatedOpenWarnings(input),
        'Viewer-ready artifact is required for --serve/--open/--open-browser; no .mvsj/.mvsx/.html artifact was available.'
      ),
    };
  }
  return await serveStory({ ...input, input: artifact });
}

async function maybeValidateOrSkip(input, artifact) {
  if (!explicitTrue(input.validate) && !explicitTrue(input.officialValidate) && !explicitTrue(input.official_validate)) return null;
  if (!artifact || !['.mvsj', '.mvsx'].includes(path.extname(artifact).toLowerCase())) {
    return {
      ok: true,
      status: 'skipped',
      reason: 'Official validation needs a built .mvsj or .mvsx artifact. Story generation was not aborted.',
    };
  }
  return await validateStory({ ...input, input: artifact });
}

async function visualizeDocking(input) {
  const viewer = normalizeString(input.viewer || 'molstory').toLowerCase();
  const resultsDir = normalizeString(input.input || input.results_dir || input.resultsDir);
  if (!resultsDir) throw new Error('input or results-dir is required');
  if (viewer === 'molstar') {
    return unsupportedCombination(`${PLANNED_OR_INTERNAL_MESSAGE} Unsupported combination: docking + molstar high-level wrapper is planned but not implemented. Use make-viewer-command for the current low-level route.`, {
      workflow: 'visualize docking',
      viewer,
      warnings: warningList(deprecatedOpenWarnings(input)),
    });
  }
  if (viewer !== 'molstory') {
    return unsupportedCombination(`Unsupported viewer for docking: ${viewer}`, {
      supported_viewers: ['molstory'],
      warnings: warningList(deprecatedOpenWarnings(input)),
    });
  }
  if (!normalizeString(input.out_dir || input.outDir)) {
    throw new Error('out-dir is required for visualize docking. Generated story source must go to an explicit user-provided output directory.');
  }
  const generation = await makeDockingStory({
    ...input,
    results_dir: resultsDir,
    reference_ligand: input.referenceLigand,
  });
  const manifest = parseJsonMaybe(generation.result?.stdout);
  const outDir = normalizeString(input.out_dir || input.outDir || manifest?.out_dir);
  let artifact = await findBuiltStoryArtifact(outDir, input.artifact);
  const warnings = warningList(deprecatedOpenWarnings(input));
  let artifactBuild = null;
  if (wantsServe(input)) {
    artifactBuild = await buildDockingStoryArtifactForViewer(input, outDir, artifact);
    if (!artifactBuild.ok) warnings.push(artifactBuild.error || 'Failed to build viewer-ready docking story artifact.');
    artifact = artifactBuild.artifact || artifact;
  }
  const validation = await maybeValidateOrSkip(input, artifact);
  const serve = await maybeServeOrSkip(input, artifact);
  const viewerUrl = serve?.viewerUrl || serve?.server?.url || null;
  const port = serve?.port || serve?.server?.port || portFromViewerUrl(viewerUrl);
  warnings.push(...warningList(artifactBuild?.warnings, validation?.warnings, serve?.warnings));
  return attachViewerResponse({
    ok: generation.ok && artifactBuild?.ok !== false && validation?.ok !== false && serve?.ok !== false,
    workflow: 'visualize docking',
    viewer,
    input: path.resolve(resultsDir),
    out_dir: outDir || null,
    artifact,
    viewerUrl,
    serverStatus: serve?.serverStatus || (serve ? serve.status || 'failed' : null),
    port,
    served: Boolean(serve?.served || serve?.server?.url),
    externalBrowserOpenRequested: wantsExternalBrowserOpen(input),
    externalBrowserOpenAttempted: Boolean(serve?.externalBrowserOpenAttempted),
    externalBrowserOpenSucceeded: Boolean(serve?.externalBrowserOpenSucceeded),
    openRequested: wantsOpen(input) || wantsExternalBrowserOpen(input),
    openAttempted: Boolean(serve?.openAttempted),
    openSucceeded: Boolean(serve?.openSucceeded),
    warnings,
    generation: summarizeExecution(generation),
    artifactBuild,
    validation,
    serve,
    note: artifact ? 'Built story artifact found.' : 'Docking story source was generated. Export to .mvsx/.mvsj before story serve or official validation.',
  }, input, {
    viewerType: viewer,
    viewerUrl,
    artifact,
...
async function visualizeStory(input) {
  const viewer = normalizeString(input.viewer || 'molstory').toLowerCase();
  if (viewer !== 'molstory') {
    return unsupportedCombination(`Unsupported viewer for story workflow: ${viewer}`, {
      supported_viewers: ['molstory'],
      warnings: warningList(deprecatedOpenWarnings(input)),
    });
  }
  const storyType = normalizeString(input.story_type || input.storyType || 'auto').toLowerCase();
  const rawInput = normalizeString(input.input || input.model || input.bcif);
  if (!rawInput) throw new Error('input is required');
  const suffix = path.extname(rawInput).toLowerCase();
  if (['.mvsj', '.mvsx', '.html'].includes(suffix)) {
    const validation = await maybeValidateOrSkip(input, path.resolve(rawInput));
    const serve = await maybeServeOrSkip(input, path.resolve(rawInput));
    const warnings = warningList(deprecatedOpenWarnings(input), validation?.warnings, serve?.warnings);
    const viewerUrl = serve?.viewerUrl || serve?.server?.url || null;
    const port = serve?.port || serve?.server?.port || portFromViewerUrl(viewerUrl);
    return attachViewerResponse({
      ok: validation?.ok !== false && serve?.ok !== false,
      workflow: 'visualize story',
      viewer,
      story_type: 'existing-artifact',
      input: path.resolve(rawInput),
      artifact: path.resolve(rawInput),
      viewerUrl,
      serverStatus: serve?.serverStatus || (serve ? serve.status || 'failed' : null),
      port,
      served: Boolean(serve?.served || serve?.server?.url),
  ...
  if (suffix === '.json') {
    return unsupportedCombination(`${PLANNED_OR_INTERNAL_MESSAGE} Unsupported combination: CellPACK recipe JSON story mode is planned but not implemented. Produce a packed coordinate model such as .bcif first.`, {
      workflow: 'visualize story',
  ...
  if (storyType === 'cellpack' || storyType === 'cellpack-mesoscale' || (storyType === 'auto' && suffix === '.bcif')) {
    if (!normalizeString(input.out_dir || input.outDir)) {
      throw new Error('out-dir is required for generated CellPACK/Mesoscale stories.');
    }
    const performanceProfile = normalizePerformanceProfile(input.performance_profile ?? input.performanceProfile, 'scientific');
    const generation = await makeCellpackStory(input);
    const manifest = parseJsonMaybe(generation.result?.stdout);
    const outDir = normalizeString(input.out_dir || input.outDir || manifest?.out_dir);
    const artifact = await findBuiltStoryArtifact(outDir, manifest?.story || input.output);
    const validation = explicitTrue(input.officialValidate) || explicitTrue(input.official_validate) || explicitTrue(input.validate)
      ? await maybeValidateOrSkip({ ...input, validate: true }, artifact)
      : null;
    const serve = await maybeServeOrSkip(input, artifact);
    const warnings = warningList(deprecatedOpenWarnings(input), validation?.warnings, serve?.warnings);
    const viewerUrl = serve?.viewerUrl || serve?.server?.url || null;
    const port = serve?.port || serve?.server?.port || portFromViewerUrl(viewerUrl);
    return attachViewerResponse({
      ok: generation.ok && validation?.ok !== false && serve?.ok !== false,
      workflow: 'visualize story',
      viewer,
      story_type: 'cellpack',
  ...
  if (storyType === 'structure' || storyType === 'generic') {
    return unsupportedCombination(`${PLANNED_OR_INTERNAL_MESSAGE} Unsupported combination: story-type ${storyType} is planned but not implemented.`, {
      workflow: 'visualize story',
      viewer,
      story_type: storyType,
      warnings: warningList(deprecatedOpenWarnings(input)),
  ...
  if (storyType === 'docking') {
    return unsupportedCombination(`${PLANNED_OR_INTERNAL_MESSAGE} Use visualize docking --viewer molstory for docking results.`, {
      workflow: 'visualize story',
      viewer,
      story_type: storyType,
      warnings: warningList(deprecatedOpenWarnings(input)),
  ...
  return unsupportedCombination(`Unsupported or ambiguous story type: ${storyType}`, {
    supported_story_types: ['auto', 'cellpack'],
  });
}

async function visualizeStructure(input) {
  const viewer = normalizeString(input.viewer || 'molstar').toLowerCase();
  const rawInput = normalizeString(input.input || input.structure || input.file);
  if (!rawInput) throw new Error('input is required for visualize structure');
  return unsupportedCombination(`${PLANNED_OR_INTERNAL_MESSAGE} Unsupported combination: structure + ${viewer} high-level wrapper is planned but not implemented.`, {
    workflow: 'visualize structure',
    input: path.resolve(rawInput),
    viewer,
    note: 'Do not route ordinary structures through CellPACK mode.',
    warnings: warningList(deprecatedOpenWarnings(input)),
  });
}

async function visualizeCommand(subject, input) {
  const target = normalizeString(subject || input._?.[0]);
  if (target === 'docking') return await visualizeDocking(input);
  if (target === 'story') return await visualizeStory(input);
  if (target === 'cellpack' || target === 'mesoscale') return await visualizeStory({ ...input, story_type: 'cellpack', viewer: 'molstory' });
  if (target === 'structure') return await visualizeStructure(input);
  throw new Error(`Unknown visualize target: ${target || '<missing>'}`);
}
...
const TOOLS = [
  ...
  {
    name: 'lp_flow_md_trajectory_serve',
    description: 'Serve a legacy/local native MD trajectory viewer from separate structure/topology and trajectory files, for example GRO + XTC. For Burrete workflow completion, use a bounded no-water multi-frame PDB display artifact and keep native files for interpretation/provenance.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        display_pdb: { type: 'string' },
        preview_topology: { type: 'string' },
        manifest: { type: 'string' },
        title: { type: 'string' },
        host: { type: 'string' },
        port: { type: 'integer' },
        display: { type: 'string', enum: ['codex-preview', 'url-only', 'external-browser'] },
        codex_preview: { type: 'boolean' },
  ...
  {
    name: 'lp_flow_make_molstar_viewer_command',
    description: 'Build the deterministic command for scripts/make_molstar_viewer.py without executing it.',
    inputSchema: {
      type: 'object',
      properties: {
        out_dir: { type: 'string' },
        receptor: { type: 'string' },
        reference_ligand: { type: 'array', items: { type: 'string' } },
        pose: { type: 'array', items: { type: 'string' } },
        top_n: { type: 'integer' },
        case: { type: 'string' },
        title: { type: 'string' },
        asset_dir: { type: 'string' },
        active_site_note: { type: 'string' },
        debug_layout: { type: 'boolean', description: 'Show Mol* debug layout panels, including left object/tree panel, sequence, and log.' },
        show_left_panel: { type: 'boolean', description: 'Alias for debug_layout.' },
        python: { type: 'string' },
      },
      required: ['out_dir', 'receptor'],
      additionalProperties: false,
    },
    handler: buildMolstarViewerCommand,
  },
  {
    name: 'lp_flow_make_docking_story_command',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Build the deterministic command for scripts/server-docking-pipeline/make_docking_story.mjs without executing it. The script creates Mol View Stories source folders from docking results.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        reference_ligand: { type: 'array', items: { type: 'string' } },
        pose: { type: 'array', items: { type: 'string' } },
        top_ligands: { type: 'integer' },
        poses_per_ligand: { type: 'string', description: 'For multi-record SDF pose files, emit up to this many pose scenes per selected ligand, or "all".' },
        center: { type: 'string' },
        active_site_note: { type: 'string' },
        node: { type: 'string' },
      },
    ...
    handler: buildDockingStoryCommand,
  },
  {
    name: 'lp_flow_make_docking_story',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Generate a Mol View Stories source folder from docking results, including story.yaml, scene markdown/JS/YAML, copied assets, README, and story_manifest.json.',
    inputSchema: {
      type: 'object',
      properties: {
        results_dir: { type: 'string' },
    ...
    handler: makeDockingStory,
  },
  {
    name: 'lp_flow_make_cellpack_story_command',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Build the deterministic command for the separate CellPACK/Mesoscale-style BinaryCIF story generator without executing it.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
        out_dir: { type: 'string' },
        output: { type: 'string' },
        annotation_map: { type: 'string' },
        title: { type: 'string' },
        assembly_id: { type: 'string' },
        cutaway: { type: 'boolean' },
        clip_axis: { type: 'string', enum: ['x', 'y', 'z'] },
        clip_offset: { type: 'number' },
        max_group_scenes: { type: 'integer' },
        performance_profile: { type: 'string', enum: ['scientific', 'overview', 'coarse-overview', 'curated-presentation'], description: 'Product-level intent. scientific=interactive inspection, overview=smooth laptop autoplay, coarse-overview builds sampled instance-center proxy primitives, curated-presentation selects real molecular instances for laptop presentation.' },
        performance_preset: { type: 'string', enum: ['fast', 'balanced', 'quality'] },
        max_proxies: { type: 'integer', description: 'Maximum instance-center proxy spheres for performance_profile=coarse-overview.' },
        proxy_radius_scale: { type: 'number', description: 'Sphere radius multiplier for performance_profile=coarse-overview.' },
        coordinate_unit: { type: 'string', enum: ['coordinate', 'angstrom', 'nanometer'] },
        guide_scale: { type: 'number' },
        builtin_validate: { type: 'boolean', description: 'Run deterministic built-in structural checks; this is not official MolViewSpec validation.' },
        validate: { type: 'boolean', description: 'Deprecated alias for builtin_validate.' },
        force_cellpack: { type: 'boolean', description: 'Override ordinary/ambiguous classification only for an independently verified CellPACK-like model.' },
        node: { type: 'string' },
      },
      required: ['input'],
      additionalProperties: false,
    },
    handler: buildCellpackStoryCommand,
  },
  {
    name: 'lp_flow_make_cellpack_story',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Generate a provenance-safe CellPACK/Mesoscale-style MVSJ story from a CellPACK-like BinaryCIF model, including exterior, cutaway, grouped component scenes, copied model, and manifest.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
    ...
    handler: makeCellpackStory,
  },
  {
    name: 'lp_flow_story_serve',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Serve a user-provided Mol View Stories artifact and return Codex/MCP-friendly viewer metadata. Does not open an external browser unless open_browser=true is explicitly provided.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
  ...
  {
    name: 'lp_flow_story_validate',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Validate a user-provided Mol View Stories artifact.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
    ...
    handler: validateStory,
  },
  {
    name: 'lp_flow_story_serve_hub',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Serve multiple user-provided Mol View Stories artifacts through one Codex-preview hub URL so the agent opens one in-app Browser tab and switches stories inside the page.',
    inputSchema: {
      type: 'object',
      properties: {
        inputs: { type: 'array', items: { type: 'string' }, description: 'Values like "Docking=C:\\path\\story.mvsx".' },
        input: { type: 'array', items: { type: 'string' }, description: 'Alias for inputs.' },
        out_dir: { type: 'string' },
        port: { type: 'integer' },
        host: { type: 'string' },
        title: { type: 'string' },
    ...
    handler: serveStoryHub,
  },
  {
    name: 'lp_flow_story_render_package',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. Create a portable server/offscreen render package for a user-provided Mol View Stories artifact. The package records the story in Chromium/Playwright and can archive WebM/MP4 outputs.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
        ...
        height: { type: 'integer' },
        seconds: { type: 'number' },
        warmup_seconds: { type: 'number' },
        scene_seconds: { type: 'number' },
        playback_mode: { type: 'string', enum: ['story', 'cinematic'] },
        require_hardware_webgl: { type: 'boolean' },
        gl: { type: 'string', enum: ['swiftshader', 'egl', 'default'] },
    ...
    handler: renderStoryPackage,
  },
  {
    name: 'lp_flow_visualize_docking',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. High-level wrapper for docking result visualization. With viewer=molstory and serve=true, returns Codex/MCP-friendly Mol* Stories viewer metadata.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
        results_dir: { type: 'string' },
        viewer: { type: 'string', enum: ['molstory', 'molstar'] },
        out_dir: { type: 'string' },
        case: { type: 'string' },
        receptor: { type: 'string' },
        reference_ligand: { type: 'array', items: { type: 'string' } },
    ...
    handler: visualizeDocking,
  },
  {
    name: 'lp_flow_visualize_story',
    description: 'Internal/compatibility only; ordinary molecular visualization goes through Burrete. High-level wrapper for existing story artifacts or supported Mol* Story inputs. With serve=true, returns Codex/MCP-friendly viewer metadata.',
    inputSchema: {
      type: 'object',
      properties: {
        input: { type: 'string' },
        viewer: { type: 'string', enum: ['molstory'] },
        story_type: { type: 'string', enum: ['auto', 'cellpack', 'cellpack-mesoscale', 'structure', 'generic', 'docking'] },
        out_dir: { type: 'string' },
        output: { type: 'string' },
        annotation_map: { type: 'string' },
        title: { type: 'string' },
        ...
        codex_preview: { type: 'boolean' },
        open_codex: { type: 'boolean' },
        builtin_validate: { type: 'boolean' },
        official_validate: { type: 'boolean' },
        validate: { type: 'boolean' },
        force_cellpack: { type: 'boolean' },
        port: { type: 'integer' },
        open_browser: { type: 'boolean' },
        node: { type: 'string' },
        timeout_ms: { type: 'integer' },
...
const INTERNAL_TOOL_DESCRIPTION_PREFIX = 'Internal/compatibility tool. Use high-level public workflow tools unless maintaining the plugin.';
const ADVANCED_CELLPACK_PROFILES = ['scientific', 'overview', 'coarse-overview', 'curated-presentation'];
const PUBLIC_CELLPACK_PROFILES = ['scientific', 'overview'];
...
function hideSchemaProperties(schema, names) {
  for (const name of names) {
    if (schema.properties && Object.hasOwn(schema.properties, name)) delete schema.properties[name];
  }
}

function visibleToolSchema(tool, mode) {
  const schema = cloneSchema(tool.inputSchema);
  if (tool.name === 'lp_flow_visualize_docking' && schema.properties?.viewer) {
    schema.properties.viewer.enum = ['molstory'];
    schema.properties.viewer.description = 'Public high-level docking viewer. Other high-level viewer routes are planned/internal and guarded at runtime.';
    if (mode === 'public') hideSchemaProperties(schema, ['serve', 'display', 'open_codex', 'mvs', 'mvs_main']);
  }
  if (tool.name === 'lp_flow_visualize_story') {
    if (schema.properties?.story_type) {
      schema.properties.story_type.enum = ['auto', 'cellpack'];
      schema.properties.story_type.description = 'Public story modes. Other story-type values are planned/internal and guarded at runtime.';
    }
    if (schema.properties?.performance_profile) {
      schema.properties.performance_profile.enum = mode === 'public' ? PUBLIC_CELLPACK_PROFILES : ADVANCED_CELLPACK_PROFILES;
      schema.properties.performance_profile.description = mode === 'public'
        ? 'Public CellPACK intent layer: scientific for interactive inspection, overview for smoother laptop preview.'
        : 'CellPACK intent layer. scientific and overview are public; coarse-overview and curated-presentation are advanced.';
    }
    if (mode === 'public') hideSchemaProperties(schema, ['serve', 'display', 'open_codex', 'performance_preset', 'max_proxies', 'proxy_radius_scale']);
  }
  if (tool.name === 'lp_flow_story_serve' && mode === 'public') {
    hideSchemaProperties(schema, ['display', 'open_codex']);
  }
  if (mode !== 'public') {
    for (const name of ['open_codex', 'open']) {
      if (schema.properties?.[name]) schema.properties[name].description = 'Deprecated compatibility alias. Prefer --codex-preview or open_browser.';
    }
  }
  return schema;
}
...
function printVisualizeHelp() {
  console.log(`LP-Flow CLI - visualize (internal/compatibility)

Usage:
  lp-flow visualize docking --input <docking_results_dir> --viewer molstory --out-dir <story_dir> [--codex-preview] [--open-browser] [--validate]
  lp-flow visualize story --input <story_or_model> --viewer molstory --story-type <auto|cellpack> --out-dir <story_dir> [--codex-preview] [--open-browser]

Status:
  Compatibility route for existing LP-Flow story scripts.
  Ordinary molecular viewing, pose review, and trajectory review should use Burrete.

Opening:
  --codex-preview starts serving and marks the viewer for in-app Codex Browser navigation.
  --open-browser starts serving, attempts to open the system browser, and still prints the URL.

Implemented now:
  docking + molstory, requiring explicit --input and --out-dir
  story/cellpack + molstory, requiring explicit --input and --out-dir

Advanced/internal:
  Use --help --internal for story hub/render-package, compatibility commands, and planned/guarded modes.

Package policy:
  The plugin contains tooling, not validation datasets. All inputs must be user-provided and generated outputs must go to --out-dir.`);
}

function printVisualizeDockingHelp() {
  console.log(`LP-Flow CLI - visualize docking

Usage:
  lp-flow visualize docking --input <docking_results_dir> --viewer molstory --out-dir <story_dir> [options]

Options:
  --input, --results-dir <dir>  Docking results folder.
  --viewer molstory            Generate a Mol View Stories docking source folder.
  --out-dir <dir>              Required output story source folder.
  --top-ligands <n>            Number of ranked ligand scenes.
  --poses-per-ligand <n|all>   For multi-record SDF files, create pose scenes per ligand and recalculate contacts for each pose.
  --center x,y,z               Optional active-site camera center.
  --active-site-note <text>    Provenance/context note.
  --codex-preview              Starts serving and returns Codex preview/browser navigation instructions.
  --open-browser               Build/serve, then attempt to open the system default browser externally.
  --validate                   Officially validate a built .mvsj/.mvsx artifact if one exists.

Notes:
  This command wraps make_docking_story.mjs and does not change docking story semantics.
  Docking story generation creates editable story source. --codex-preview/--open-browser require a viewer-ready .mvsx/.mvsj artifact and return non-zero if export fails.
  --open-browser failure is reported as a warning and the URL is printed as fallback.
  Deprecated preview aliases remain callable in compatibility mode and emit runtime warnings.
  No old generated stories or packaged validation fixtures are used as input.`);
}

function printVisualizeStructureHelp() {
  console.log(`LP-Flow CLI - visualize structure

Usage:
  lp-flow visualize structure --input <protein.pdb|model.cif|model.bcif|ligand.sdf> --viewer molstar [--out-dir <dir>]

Status:
  Planned high-level wrapper. Do not force ordinary structures into CellPACK/Mesoscale mode.

Current route:
  Use lower-level Mol* viewer generation commands when the receptor/pose roles are known.
  Ordinary structures are not routed to CellPACK automatically.`);
}

function printVisualizeStoryHelp() {
  console.log(`LP-Flow CLI - visualize story

Usage:
  lp-flow visualize story --input <story_or_model> --viewer molstory --story-type <auto|cellpack> [options]
  lp-flow visualize story --input <packed.bcif> --viewer molstory --story-type cellpack --out-dir <story_dir> [--codex-preview] [--open-browser]

Options:
  --input <path>               Existing .mvsj/.mvsx/.html artifact or supported model.
  --viewer molstory            Required for story workflow.
  --story-type <type>          auto, cellpack.
  --out-dir <dir>              Required output directory for generated stories.
  --performance-profile scientific|overview
                                CellPACK intent layer: scientific is for interactive inspection;
                                overview is for smoother laptop preview.
  --builtin-validate true      Run generator built-in checks where supported.
  --official-validate true     Run Deno-based Mol* validation when Deno is available.
  --force-cellpack true        Override CellPACK classifier only when independently justified.
  --codex-preview              Starts serving and returns Codex preview/browser navigation instructions.
  --open-browser               Starts serving, attempts to open the system browser externally, and prints the URL as fallback.

Implemented:
  existing .mvsj/.mvsx/.html serve/validate
  packed CellPACK/Mesoscale .bcif via make_cellpack_story.mjs

Unsupported:
  CellPACK recipe JSON; produce a packed coordinate model first.
  ordinary-structure story generation is planned.
  Other story-type values are planned/internal and guarded at runtime.

Advanced:
  Use --help --advanced for coarse-overview and curated-presentation profiles.
  Deprecated preview aliases remain callable in compatibility mode and emit runtime warnings.

Package policy:
  Example URLs may be documented, but packed .bcif/story artifacts are not bundled or used as defaults.`);
}

function printStoryHelp() {
  console.log(`LP-Flow CLI - story

Usage:
  lp-flow story serve --input <story.mvsj|story.mvsx> [--port 8765] [--open-browser]
  lp-flow story validate --input <story.mvsj|story.mvsx> [--official] [--deno <deno>]

Advanced:
  story serve-hub and story render-package are available in --help --internal.

Runtime policy:
  Node scripts run with the plugin Node runtime.
  validate_mvsj_official.mjs is Deno-based and is never run with Node.
  If Deno is unavailable, official validation is reported as skipped.
  Validation is user-input-based; no built-in validation datasets are loaded.`);
}

function printStoryServeHelp() {
  console.log(`LP-Flow CLI - story serve

Usage:
  lp-flow story serve --input <story.mvsj|story.mvsx> [options]

Options:
  --input <path>       Story artifact. Existing html/story-dir support is kept only as compatibility.
  --port <number>     Preferred local port. Default is chosen by serve_mvs_story.mjs.
  --host <host>       Bind host.
  --title <text>      Browser page title.
  --codex-preview     Mark this viewer for in-app Codex Browser preview and imply serving.
  --open-browser      Attempt to open the system default browser after the server starts.

Output:
  JSON with kind="molstar-viewer", viewerType, displayIntent, artifact path, viewer URL, served status, port, externalBrowserOpenRequested, warnings, viewerResource, and command metadata.
  displayIntent="open-in-codex-preview" tells capable Codex hosts that the URL is intended for an interactive preview. The plugin cannot force preview opening if the host/session has no preview/navigation capability.
  In a Codex agent session with Browser plugin access, --codex-preview is a request to show the in-app browser and navigate it to viewerUrl.
  If external browser opening fails, the server stays running and the URL is printed as fallback.
  Deprecated preview aliases remain callable in compatibility mode and emit runtime warnings.
  The input story must be supplied by the user or generated in a user-selected --out-dir.`);
}

function printStoryServeHubHelp() {
  console.log(`LP-Flow CLI - story serve-hub

Usage:
  lp-flow story serve-hub --inputs "StoryA=<story.mvsx>" --inputs "StoryB=<story.mvsj>" [options]

Options:
  --inputs <label=path>  Repeatable story artifact input. Labels become hub buttons/tabs.
  --input <label=path>   Alias for --inputs.
  --out-dir <dir>        Optional hub output directory. Default: temporary user-local directory outside plugin source.
  --port <number>        Preferred first local port. Child viewers and hub use nearby free ports.
  --host <host>          Bind host. Default: 127.0.0.1.
  --title <text>         Hub page title.
  --display <mode>       codex-preview, url-only, or external-browser metadata. Default: codex-preview.
  --codex-preview        Mark the hub URL for in-app Codex Browser preview.
  --open-codex           Deprecated alias for --codex-preview. Never opens the system/default browser.

Behavior:
  Starts one viewer server per user-provided story and one lightweight hub page with buttons/tabs.
  JSON returns kind="molstar-viewer-hub", hubUrl, viewerUrls, and codexPreview.openOnly="hubUrl".
  Agents should open only hubUrl in the current Codex in-app Browser tab, then switch stories inside the hub.
  Do not open each viewerUrl as a separate Codex tab unless the user explicitly asks for multiple tabs.
  No bundled validation datasets are used and no external browser is opened.`);
}

function printStoryRenderPackageHelp() {
  console.log(`LP-Flow CLI - story render-package

Usage:
  lp-flow story render-package --input <story.mvsj|story.mvsx> --out-dir <dir> [options]

Options:
  --input <path>       User-provided Mol View Stories artifact.
  --out-dir <dir>      Output directory for the portable render package.
  --width <px>         Recording width. Default: 1920.
  --height <px>        Recording height. Default: 1080.
  --seconds <n>        Recording duration. Default: 30.
  --warmup-seconds <n> Wait after viewer load before final trimmed video starts. Default: 20.
  --scene-seconds <n>  Seconds before each automatic Next click. Default: 5.
  --playback-mode <m>  story or cinematic. Cinematic keeps one loaded scene and animates camera. Default: story.
  --require-hardware-webgl true
                       Abort instead of producing a stuttering software-WebGL video.
  --archive            Also create story_render_package.tar.gz for upload/download.
  --playwright <pkg>   Playwright package name/path used on the render host. Default: playwright.
  --ffmpeg <path>      ffmpeg executable used on the render host. Default: ffmpeg.

Behavior:
  Creates story_render_package/ with render_story_video.mjs, the story artifact, run_render.sh, run_render.ps1, and a manifest.
  The render host must provide Node, Playwright/Chromium, WebGL/EGL-capable rendering, and optionally ffmpeg.
  For smooth CellPACK videos, use --playback-mode cinematic --require-hardware-webgl true. If Chromium reports SwiftShader, fix server graphics access before rendering.
  This command does not use bundled validation datasets, does not open a browser, and does not change story semantics.`);
}

function printStoryValidateHelp() {
  console.log(`LP-Flow CLI - story validate

Usage:
  lp-flow story validate --input <story.mvsj|story.mvsx> [--official] [--deno <deno>]

Behavior:
  Runs official Mol* MVSData validation with Deno.
  Never invokes validate_mvsj_official.mjs with Node.
  If Deno is unavailable, returns ok=true with official_validation.status="skipped".
  For .mvsx input, extracts index.mvsj into a temporary folder before validation.
  No bundled validation datasets are used.`);
}
...
Public workflows:
  ...
  node scripts/lp-flow.mjs md <connect|submit|status|log|result|analyze-tpr|trajectory-serve> ...
...
Compatibility preview aliases:
  --open-codex            Deprecated alias for --codex-preview.
  --open                  Deprecated alias for serving. It does not open the system browser.
...
Legacy narrative-viewer compatibility commands are available only in --help --internal and are hidden from normal/advanced help.
Interactive molecular viewing should be handed off to Burrete.`);
}
...
Internal/compatibility commands:
  ...
  node scripts/lp-flow.mjs make-viewer-command --out-dir <dir> --receptor <file> [--pose label=file] [--debug-layout]
  node scripts/lp-flow.mjs make-docking-story-command --results-dir <results-dir>
  node scripts/lp-flow.mjs make-docking-story --results-dir <results-dir> [--out-dir <story-dir>]
  node scripts/lp-flow.mjs make-cellpack-story-command --input <model.bcif>
  node scripts/lp-flow.mjs make-cellpack-story --input <model.bcif> [--out-dir <story-dir>] [--annotation-map <map.json>]
  node scripts/lp-flow.mjs build-story-artifact --source-dir <story-source-dir> --output <story.mvsx>
  ...
  node scripts/lp-flow.mjs visualize docking --input <results-dir> --viewer molstory --out-dir <story-dir> [--codex-preview] [--open-browser]
  node scripts/lp-flow.mjs visualize story --input <story-or-packed.bcif> --viewer molstory --story-type <auto|cellpack> --out-dir <story-dir> [--codex-preview] [--open-browser]
  node scripts/lp-flow.mjs story serve --input <story.mvsj|story.mvsx> [--codex-preview] [--open-browser]
  node scripts/lp-flow.mjs story validate --input <story.mvsj|story.mvsx> [--official]
  node scripts/lp-flow.mjs story serve-hub --inputs "Label=<story.mvsj|story.mvsx>" [--inputs ...] [--codex-preview]
  node scripts/lp-flow.mjs story render-package --input <story.mvsj|story.mvsx> --out-dir <dir> [--archive]
  node scripts/lp-flow.mjs list-tools --internal

Planned/hidden guarded modes:
  visualize structure
  visualize docking --viewer molstar
  visualize story --story-type generic|structure|docking

Runtime guard text:
  ${PLANNED_OR_INTERNAL_MESSAGE}

Deprecated hidden aliases:
  --open-codex            Alias for --codex-preview with runtime warning.
  --open                  Alias for serving with runtime warning.
...
More help:
  ...
  --help --internal       Compatibility commands and planned/guarded modes.
...
Visualization policy:
  ...
  Legacy narrative-viewer commands are compatibility-only and hidden from public discovery.
...
Usage:
  ...
  node scripts/lp-flow.mjs md trajectory-serve --structure <reference.gro|pdb|cif> --trajectory <trajectory.xtc|trr|dcd> [--codex-preview]
...
Policy:
  ...
  trajectory-serve is a legacy/local native viewer for real structure + trajectory files.
...
async function main() {
  ...
  if (command === 'md') {
    ...
    else if (action === 'trajectory-serve' || action === 'serve-trajectory') result = await serveMdTrajectory(args);
  ...
  if (command === 'visualize') {
    ...
    if (targetRest.includes('--help') || targetRest.includes('-h')) {
      if (target === 'docking') printVisualizeDockingHelp();
      else if (target === 'structure') printVisualizeStructureHelp();
      else if (target === 'story' || target === 'cellpack' || target === 'mesoscale') printVisualizeStoryHelp();
      else throw new Error(`Unknown visualize target: ${target}`);
      return;
    }
    const args = parseCliArgs(targetRest);
    const result = await visualizeCommand(target, args);
    console.log(jsonText(result));
    if (result?.ok === false) process.exitCode = 1;
    return;
  }
  if (command === 'story') {
    ...
    if (actionRest.includes('--help') || actionRest.includes('-h')) {
      if (action === 'serve') printStoryServeHelp();
      else if (action === 'serve-hub') printStoryServeHubHelp();
      else if (action === 'render-package') printStoryRenderPackageHelp();
      else if (action === 'validate') printStoryValidateHelp();
      else throw new Error(`Unknown story action: ${action}`);
      return;
    }
    const args = parseCliArgs(actionRest);
    let result;
    if (action === 'serve') result = await serveStory(args);
    else if (action === 'serve-hub') result = await serveStoryHub(args);
    else if (action === 'render-package') result = await renderStoryPackage(args);
    else if (action === 'validate') result = await validateStory(args);
    else throw new Error(`Unknown story action: ${action}`);
    console.log(jsonText(result));
    if (result?.ok === false) process.exitCode = 1;
    return;
  ...
  else if (command === 'make-viewer-command') result = buildMolstarViewerCommand({ ...args, pose: args.pose, reference_ligand: args.referenceLigand });
  else if (command === 'make-docking-story-command') result = buildDockingStoryCommand({ ...args, pose: args.pose, reference_ligand: args.referenceLigand });
  else if (command === 'make-docking-story') result = await makeDockingStory({ ...args, pose: args.pose, reference_ligand: args.referenceLigand });
  else if (command === 'make-cellpack-story-command') result = buildCellpackStoryCommand(args);
  else if (command === 'make-cellpack-story') result = await makeCellpackStory(args);
...
export {
  buildDockingPayload,
  buildDockingStoryCommand,
  buildCellpackStoryCommand,
  buildMolstarViewerCommand,
