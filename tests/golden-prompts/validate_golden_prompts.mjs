...
for (const [index, item] of (data.cases || []).entries()) {
  ...
  assert(!mojibake.test(item.prompt || ''), `${label}.prompt contains mojibake or replacement characters`);
...
const russian = data.cases.find(item => item.id === 'full_6lr9_russian_pipeline');
...
assert(/Сделай редокинг/u.test(russian?.prompt || ''), '6LR9 case must preserve its Russian trigger text');
